...
public abstract class Player extends Actor {
  ...
  // The time the whole team has, in milliseconds. Each player is individually capped on time, not
  // the team
  ...
  public Location evade() {
    for (Location loc : getGrid().getOccupiedAdjacentLocations(getLocation()))
      if (hasFlag()
          && getGrid().get(loc) instanceof Player
          && ((Player) getGrid().get(loc)).getTeam().equals(getOtherTeam()))
  ...
  public Location intruderSearch() {
    for (Location loc : getGrid().getOccupiedLocations())
      if (getGrid().get(loc) instanceof Player
          && ((Player) getGrid().get(loc)).getTeam().equals(getOtherTeam())
          && !getOtherTeam().onSide(loc)) return loc;
    return null;
  }
  ...
  public Location getImmediateObjectiveLocation(Location loc) {
    if (getGrid().get(loc) instanceof Player
        && ((Player) getGrid().get(loc)).getTeam() != this.getTeam()
        && ((Player) getGrid().get(loc)).getTeam().getSide() == getMyTeam().getSide())
      return getOtherTeam().onSide(loc)
          ? getGrid()
              .getEmptyAdjacentLocations(getLocation())
              .get(
                  (int) (Math.random() * getGrid().getEmptyAdjacentLocations(getLocation()).size()))
          : loc;
    else if (getGrid().get(loc) instanceof Rock)
      return getGrid()
          .getEmptyAdjacentLocations(getLocation())
          .get((int) (Math.random() * getGrid().getEmptyAdjacentLocations(getLocation()).size()));
    else if (getGrid().get(loc) instanceof Flag
        && ((Flag) getGrid().get(loc)).getTeam() != this.getTeam()) return loc;
    else
      return null; // the reason we return null is to allow for several iterations of this method to
    // be called in a row.
  }
