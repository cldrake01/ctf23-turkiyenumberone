...
public abstract class Team {
  ...
  protected final void addTeamToGrid(Grid<Actor> grid, int side) {
    ...
    for (Player player : players) {
      double dist =
          Math.sqrt(
              Math.pow(player.getStartLocation().getRow() - DEFAULT_FLAG_LOCATION.getRow(), 2)
                  + Math.pow(
                      player.getStartLocation().getCol() - DEFAULT_FLAG_LOCATION.getCol(), 2));
      if (player.getStartLocation().getCol() >= grid.getNumCols() / 2
          || player.getStartLocation().getCol() < 0
          || dist < 10.0) {
  ...
  public void displayStats(int steps) {
    System.out.println(
        name
            ...
            + " tags. They made "
            + offensiveMoves
            + " steps on the offencive side, and were on the other side "
            + offensiveMoves / (steps + players.size())
            + "% of the game");
  }
  ...
  public final Location adjustForSide(Location loc, Grid<Actor> grid) {
    return new Location(
        loc.getRow(), (side == 0 ? loc.getCol() : grid.getNumCols() - 1 - loc.getCol()));
  }
  ...
  public final boolean onSide(Location loc) {
    return side == 0 && loc.getCol() < grid.getNumCols() / 2
        || side == 1 && loc.getCol() >= grid.getNumCols() / 2;
  }
  ...
  public final boolean nearFlag(Location loc) {
    ...
    return Math.abs(loc.getRow() - fLoc.getRow()) <= RANGE
        && Math.abs(loc.getCol() - fLoc.getCol()) <= RANGE;
    // return Math.sqrt(Math.pow(loc.getRow() - flag.getLocation().getRow(), 2) +
    // Math.pow(loc.getCol() - flag.getLocation().getCol(), 2)) <= 4;
  }
