...
public class WorldFrame<T> extends JFrame {
    ...
    addWindowListener(
        new WindowAdapter() {
          public void windowClosing(WindowEvent event) {
            count--;
            if (count == 0 && SHUTDOWN_ON_CLOSE) System.exit(0);
          }
        });
      ...
      if (accel.startsWith(metaPrefix)) {
        ...
        KeyStroke key =
            KeyStroke.getKeyStroke(
                KeyStroke.getKeyStroke(accel.substring(metaPrefix.length())).getKeyCode(),
                menuMask);
  ...
  public void setRunMenuItemsEnabled(boolean enable) {
    for (JMenuItem item : menuItemsDisabledDuringRun) item.setEnabled(enable);
  }
  ...
  private void showAboutPanel() {
    String html =
        MessageFormat.format(
            resources.getString("dialog.about.text"), resources.getString("version.id"));
    String[] props = {
      "java.version",
      "java.vendor",
      "java.home",
