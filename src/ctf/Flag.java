...
/**
 * Special Actor that acts as the Flag in a CTF Game. If placed in the Grid, it is available to be
 * picked up. If it exists, but is not in a Grid that means a Player has picked it up and is
...
public class Flag extends Actor {
  ...
  /**
   * Constructs a new Flag. This is only done when a Team is created and is only allowed by the CTF
   * Team superclass.
   *
   * @param team
   */
  ...
  /**
   * Adds the Flag to a Grid, meaning that it is available to be picked up. This can only be done by
   * the ActorWorld class at Construction time or by the Player superclass when a Flag is dropped
   * and becomes available to pick up again.
   *
  ...
  public final void putSelfInGrid(Grid<Actor> grid, Location loc) {
    ...
    if (callingClass.equals("info.gridworld.actor.ActorWorld")
        || callingClass.equals("ctf.Player")) {
   ...
   *
   * @return the Location of the Flag whether it is being carried
   */
  ...
  /**
   * Returns an indication of whether the Flag is currently being carried
   *
   * @return whether the Flag is currently being carried
   */
