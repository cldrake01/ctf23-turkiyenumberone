...
public abstract class Player extends Actor {
  ...
  private static final int TURN_MAX_TIME =
      100; // The time in milliseconds a player has to calculate a move Loc
  ...
  /**
   * This is called each time a Player is called on to act. The Player follows this sequence: 1)
   * Check to see if either team has already won 2) If Player has recently been tagged out, it must
   * wait until the cool-down period has expired 3) If the Player can act, it processes its
   * immediate neighbors. It can: - pick up an adjacent Flag - tag an opponent. If the opponenet has
   * the Flag, it will always tag out the opponent. if the opponenet does not have the Flag, it will
   * only tag out the opponent with a probability based on how many neighbors are on the other team
   * (to decrease the liklihood of multiple tags) 4) Calculates the Location to move to by calling
   * getMoveLocation. If it takes too much time to get the move Location, it may not move at all 5)
   * Moves to the new Location. The new Location must be: - valid - adjacent to its current Location
   * - not too close to its own Flag
   *
   * <p>This method can only be called by CTFWorld
   */
  ...
  private void makeMove(Location loc) {
    ...
    if (team.onSide(getLocation())
        && getGrid().get(team.getFlag().getLocation()) instanceof Flag
        && team.nearFlag(getLocation())
        && team.nearFlag(loc)) {
      loc = bounce();
  ...
  private void tag() {
    ...
    if (callingClass.equals("ctf.Player")) {
      ...
      do {
        nextLoc =
            team.adjustForSide(
                new Location((int) (Math.random() * getGrid().getNumRows()), 0), getGrid());
   ...
   *
   * <p>This may ONLY be called automatically by CTFWorld - it can not be called by other Player
   * classes
   *
  ...
  /**
   * Removes a Player from its Grid This may ONLY be called automatically by CTFWorld - it can not
   * be called by other Player classes
   */
  ...
  /**
   * Moves this Player to the specified Location. This may ONLY be called indirectly by the Player
   * superclass and not directly by any subclasses
   *
