...
/**
 * CTFWorld is a custom ActorWorld that consists of two Teams that play Capture the Flag until
 * either 1000 steps have been made or one team captures the opposing Team's Flag and returns it to
 * their own side.
 */
public final class CTFWorld extends ActorWorld {
  ...
  private final Team rightTeam; // the two teams playing
   ...
   * @param rightTeam the Team that will appear on the right side of the field (it will
   *     automatically be mirrored from the left side to the right side
   */
  ...
  /**
   * Constructs a new Capture the Flag World with the two given Teams and the specified number of
   * Rocks.
   *
   * @param leftTeam
   * @param rightTeam
  ...
  public CTFWorld(Team leftTeam, Team rightTeam, int numClumps, int clumpsize) {
    ...
    for (int i = 0; i < numClumps; i++) {
      ...
      } while (nearFlag(grid, rockClumpLocation)
          || nearPlayer(leftTeam, rockClumpLocation)
          || nearPlayer(rightTeam, rockClumpLocation)
          || grid.get(rockClumpLocation) != null);
      ...
      for (int j = 0; j < clumpsize - 1; j++) {
        ...
        if (grid.isValid(possibleRockLocation)
          ...
          if (!nearPlayer(leftTeam, possibleRockLocation)
              && !nearPlayer(rightTeam, possibleRockLocation))
